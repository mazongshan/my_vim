...
map <F7> :make -Bj8;make install<CR><CR><CR> :copen<CR><CR>
" 按下F8，光标移到上一个错误所在的行
map <F8> :cp<CR>
" 按下F9，光标移到下一个错误所在的行
map <F9> :cn<CR>
...
"<2>、用vim打开某个源码文件，末行模式下，输入“:cs add cscope.out"（该命令已被我们映射为快捷键F4），添加"cscope数据库到vim。因为我们已将vim配置为启动时，自动添"加当前目录下的cscope数据库，所以你再添加该"cscope数据库时，vim会提示“重复cscope数据库 未被加入“
"<3>、完成前两步后，现在就可以用“cs find c”等Cscope查找命令查找关键字了。我们已在.vimrc中将“cs find c”等Cscope查找命令映射为<C-_>c等快捷键（按法是先按"Ctrl"+Shift+-, 然后很快按下c）
